...
class Controller extends Feature {
  /**
  ...
  getHandler (req, res) {
    ...
    switch (params[0]) {
      case '/api/map':
        response = this.robot.brain.navigation.saveWorld()
        break
      case '/api/commands':
      ...
      case '/api/info':
        ...
        break
      case '/map':
        response = this.robot.brain.navigation.asSvgMap()
        break
      default:
        response = 'do what??'
    }
  ...
  handler (req, res) {
    ...
    command.origin = Origin.HUMAN
    command.notes = 'from API'
