...
class Brain extends Feature {
  ...
  async init() {
    ...
    this.speak('Sound Card enabled: check! Camera enabled: check! Navigation enabled: check! Serial expansion enabled: check! Power management enabled: check! Inertial measurement unit: enabled! Compass enabled: check!, Light sensors enabled: check!')
    this.sound('STARTUP2')
  }
