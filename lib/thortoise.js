...
class Thortoise {
  ...
  /**
   * It creates a new Wheel collection, and then deletes the wheelSettings
   */
  init () {
    this.brain.init()
    console.log(this.wheelSettings)
    this.wheels = new Wheels(this.wheelSettings)
    delete this.wheelSettings // we don't need them now
  }
  ...
  turn (command) {
    ...
    if (this.brain.navigation.matchedBearing(this.desiredBearing)) {
      ...
      this.wheels[this.direction]() // turn left or right
