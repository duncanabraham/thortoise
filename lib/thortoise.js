      ...
      switch (nextAction.type) {
        case 'action':
          switch (nextAction.action) {
            ...
            case 'south':
            case 'west':
            // Walk needs to work out where we are walking to using the current direction and the current bearing
            // the result should be a grid coordinate to move to using goto
            case 'walk':
            // Goto needs to provide coords then use navigation.solve() to work out the next position we're moving to
            case 'goto':
              break
          }
          break
