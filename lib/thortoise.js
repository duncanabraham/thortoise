...
class Thortoise {
  ...
  _tenPerSecond () {
    // this.brain.imu.getAllData() // brain is taking care of this
  }
  ...
  get export () {
    ...
    exportObj.navigation = this.brain.navigation
    delete exportObj.navigation.grid
    delete exportObj.navigation.gps.parser
    delete exportObj.navigation.running
    delete exportObj.navigation.imu.running
    delete exportObj.navigation.compass.running
    delete exportObj.running
    delete exportObj.sleeping
    delete exportObj.store
    delete exportObj.counters
    delete exportObj.registry
    delete exportObj.heartbeat
   ...
   */
  init () {
    // TODO: Create the servo instances here and pass the servo references to the leg rather than the options
    this.brain.init()
    this.legs = []
    this.legSettings.forEach(legSettings => {
      const hipServo = this.servoController.addServo(legSettings.hipServoSettings)
      const femurServo = this.servoController.addServo(legSettings.femurServoSettings)
      const kneeServo = this.servoController.addServo(legSettings.kneeServoSettings)
      this.legs.push(new Leg({ legSettings, hipServo, femurServo, kneeServo }))
    })

    // this.legs = this.legSettings.map(thisLeg => new Leg({ ...thisLeg, steps: this.steps, this.servoController }))
    // // this.legs = []
    // // this.legs.push(new Leg({ ...this.legSettings[0], steps: this.steps })) // front-left
    // // this.legs.push(new Leg({ ...this.legSettings[1], steps: this.steps })) // front-right
    // // this.legs.push(new Leg({ ...this.legSettings[2], steps: this.steps })) // back-left
    // // this.legs.push(new Leg({ ...this.legSettings[3], steps: this.steps })) // back-right
  ...
  _doMovement () {
    ...
    this.legs.forEach(l => l.nextStep(this.counters.steps.tick))
  }
   ...
   */
  _verbose () {
    if (this.verbose && this.legs[0].newAngles && this.legs[0].newAngles.x) {
