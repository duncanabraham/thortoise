...
const { Command, Origin } = require('./command')
...
class Thortoise {
  constructor (options) {
    ...
    if (process.env.ENVIRONMENT === 'live') {
      process.on('SIGINT', () => {
        this.shutdown('Received SIGINT. Cleaning up before exit.', 0)
      })

      process.on('uncaughtException', (err) => {
        console.error(`Uncaught Exception: ${err}`)
        this.shutdown('Uncaught Exception', 1)
      })

      process.on('unhandledRejection', (reason, p) => {
        console.error(`Unhandled Rejection at: Promise ${p}, reason: ${reason}`)
        this.shutdown('unhandled rejection!', 1)
      })
    }
  }

  async powerManagement () { // TODO: why didn't the Ticker work?
    console.log('Checking Power Status')
    this.brain.sound('CLICK1', 0.1)
  ...
  shutdown (message, code) {
    this.brain.speak('Thortoise is shutting down, please stand by')
    log.info(message)
    this.brain.speak(message)
  ...
  _addImmediateAction (action, other = {}) {
    this.action = new Command({ type: 'action', name: action, action, origin: Origin.MACHINE, ...other })
