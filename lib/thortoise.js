...
class Thortoise {
  constructor (options) {
    ...
    const leg1 = new Pos(this.hipServo.startAt, this.femurServo.startAt, this.kneeServo.startAt, 'front-right', 0, 100, 100)
    const leg2 = new Pos(this.hipServo.startAt, this.femurServo.startAt, this.kneeServo.startAt, 'front-left', Math.PI / 2, 100, 100)
    const leg3 = new Pos(this.hipServo.startAt, this.femurServo.startAt, this.kneeServo.startAt, 'back-right', Math.PI, 100, 100)
    const leg4 = new Pos(this.hipServo.startAt, this.femurServo.startAt, this.kneeServo.startAt, 'back-left', Math.PI * 1.5, 100, 100)
    ...
    driver.exitHandler(this.exitHandler.bind(this))
  }
  ...
  init () {
    ...
    this.legs[0].servos.hip.sweep()
  }

  _setLegsToPosition () {
    const { l1, l2, l3, l4 } = this.legPositions
    console.log('set positions: ', l1)
  ...
  runLoop () {
    ...
    this.walk.tock()
    this.legPositions = this.walk.nextStep(this.direction, this.tick)
  }

  start () {
    console.log('Starting main loop...')
    const loopSpeedMS = this.speedInMS / 10 // 10 times per second
