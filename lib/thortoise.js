  ...
  runLoop () {
    ...
    if (this.startupSequence) {
      ...
      this.legs.forEach(leg => {
        ...
        const maxTrip = new Triplet(leg.hip.maxPos, leg.femur.maxPos, leg.knee.maxPos)
        leg.setAngles(minTrip)
          .then(() => {
            console.log(`${leg.name} moving backand back`)
            leg.setAngles(maxTrip)
          })
      })
    } else {
