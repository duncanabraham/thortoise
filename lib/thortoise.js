...
// const MotorController = require('./odrive')
...
class Thortoise {
  constructor (options) {
    ...
    // this.motorController = new MotorController('/dev/ttyACM0')
  ...
  async exitHandler () {
    this.tracks.stop()
    // await this.motorController.zeroMotors()
    // await this.motorController.disableMotors()
    this.store.append('INFO', 'Stopped')
  }

  messageHandler(channel, message) {
    // TODO:  need to send the message to the correct sensor which will be of type "externalDevice"
    console.log(`Received: Ch: ${channel}  message: ${message}`)
  }
   ...
   */
  async init () {
    this.redisClient.on('message', this.messageHandler.bind(this))
    this.redisClient.subscribe('sensor_data')
    this.brain.init()
    // await this.odrive.init()
    delete this.trackSettings // we don't need them now
  }
  ...
  turn (command) {
    ...
    if (this.brain.navigation.matchedBearing(this.desiredBearing)) {
      ...
      this.tracks[this.direction]() // turn left or right
