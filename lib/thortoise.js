...
class Thortoise {
  ...
  init () {
    ...
    this.legs = []
    const frontLeft = this.legSettings[0]
    const frontRight = this.legSettings[1]

    // const backLeft = this.legSettings[2]
    // const backRight = this.legSettings[3]

    this.legs.push(new Leg({ ...frontLeft, driver: this.driver, steps: this.steps }))
    this.legs.push(new Leg({ ...frontRight, driver: this.driver, steps: this.steps }))
    // this.legs.push(new Leg({ ...backLeft, driver: this.driver, steps: this.steps }))
    // this.legs.push(new Leg({ ...backRight, driver: this.driver, steps: this.steps }))
  ...
  _addAction (action, notes) {
    this.action = new Command({ type: 'action', name: action, action, origin: 'machine', notes })
  ...
  turn (bearing) {
    ...
    if (this.brain.navigation.matchedBearing(this.desiredBearing)) {
      ...
      this._addAction('scan', 'have a look around')
    } else {
      this._addAction('turn', `turning ${this.direction} ${this.turnDirection}`) // keep on turning
    }
  }
