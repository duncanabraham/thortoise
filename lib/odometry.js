...
class Odometry {
  ...
  async send () {
    if (!this.sendingData && this.odomData?.length && serverUrl.length) { // skip this if there's no data or we don't have a log server configured
      try {
        this.sendingData = true
        if (!keyManager.apiKey || keyManager.isApiKeyExpired()) {
          console.log('Calling token refresh: ', JSON.stringify(keyManager))
          await keyManager.refreshApiKey()
        }
        const response = await httpClient.post(`${serverUrl}/api/odometry`, this.odomData, {
