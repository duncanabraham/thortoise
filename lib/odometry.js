const { keyManagerSettings, options: _options } = require('../config')
...
class Odometry {
  ...
  async send () {
    ...
    if (this.env !== 'dev' && !this.sendingData && data.length && serverUrl.length) { // skip this if there's no data or we don't have a log server configured
      try {
