...
class ICM20948 {
  ...
  swReset () {
    ...
    let register = this._readByte(this.REG_ADD_PWR_MIGMT_1)
    register = register | 0x80 // set bit 7
    this._writeByte(this.REG_ADD_PWR_MIGMT_1, register)
  }
  ...
  async readCompass () {
    // user bank 0 register
    this.setBank(this.REG_VAL_REG_BANK_0)
    const x = this._signedWord(this._readHL(this.REG_ADD_MAG_XOUT_H))
    const y = this._signedWord(this._readHL(this.REG_ADD_MAG_YOUT_H))
    const z = this._signedWord(this._readHL(this.REG_ADD_MAG_ZOUT_H))
