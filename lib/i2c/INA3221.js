...
const registers = require('./registers/INA3221_Registers')
const { toN } = require('../utils')

class INA3221 extends i2cBase {
  static channels = [1, 2, 3]
  ...
  _init () {
    this.config = this.INA3221_CONFIG_ENABLE_CHAN1 | this.INA3221_CONFIG_ENABLE_CHAN2 | this.INA3221_CONFIG_ENABLE_CHAN3 |
      this.INA3221_CONFIG_AVG1 |
      this.INA3221_CONFIG_VBUS_CT2 |
      this.INA3221_CONFIG_VSH_CT2 |
      this.INA3221_CONFIG_MODE_2 | this.INA3221_CONFIG_MODE_1 | this.INA3221_CONFIG_MODE_0
  ...
  getCurrent (channel) {
    const valueDec = this.getShuntVoltage(channel) / this.SHUNT_RESISTOR_VALUE
    return valueDec
  }

  async readValue () {
    const result = {}
    INA3221.channels.forEach(channel => {
      const channelResult = {
        busVoltage: toN(this.getBusVoltage(channel), 3),
        shuntVoltage: toN(this.getShuntVoltage(channel), 3),
        current: toN(this.getCurrent(channel), 3)
      }
      result[this.channelNames[channel - 1]] = channelResult
    })
    this.value = result
    return result
  }
