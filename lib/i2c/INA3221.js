...
class INA3221 extends i2cBase {
  ...
  async readValue () {
    ...
    ;[1, 2, 3].forEach(channel => {
      const channelResult = {
        busVoltage: toThree(this.getBusVoltage(channel)),
        shuntVoltage: toThree(this.getShuntVoltage(channel)),
        current: toThree(this.getCurrent(channel))
      }
