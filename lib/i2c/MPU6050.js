...
class MPU6050 extends i2cBase {
  ...
  run () {
    this.running = setInterval(this.getAllData, 100)
  }
  ...
  _getAccelRange () {
    const rawData = this._readByte(this.ACCEL_CONFIG)
  ...
  _getGyroRange () {
    const rawData = this._readByte(this.GYRO_CONFIG)
  ...
  getAllData () {
    ...
    this.value.accel = this.getAccelData()
