...
class UPS extends i2cBase {
  ...
  async readData() {
    try {
      this._writeWord(REG_CALIBRATION, this._cal_value)
      let shuntVoltage = this._readWord(REG_SHUNTVOLTAGE)
      let busVoltage = this._readWord(REG_BUSVOLTAGE)
      let current = this._readWord(REG_CURRENT)
      let power = this._readWord(REG_POWER)

      if (shuntVoltage > 32767) shuntVoltage -= 65535
      if (current > 32767) current -= 65535
      if (power > 32767) power -= 65535

      // shuntVoltage *= 0.01
      // busVoltage = (busVoltage >> 3) * 0.004
      // current *= this._current_lsb
      // power *= this._power_lsb
