...
class UPS extends i2cBase {
  ...
  async readData() {
    try {
      this._writeWord(REG_CALIBRATION, this._cal_value)
      let shuntVoltage = this._twosComp(this._readHL(REG_SHUNTVOLTAGE))
      console.log('raw shuntVoltage = ', shuntVoltage)
      shuntVoltage = shuntVoltage * 0.01 / 1000

      this._writeWord(REG_CALIBRATION, this._cal_value)
      let busVoltage = (this._readLH(REG_BUSVOLTAGE))
      console.log('raw busVoltage = ', busVoltage)
      busVoltage = (busVoltage / 8) * 0.004

      this._writeWord(REG_CALIBRATION, this._cal_value)
      let current = this._twosComp(this._readHL(REG_CURRENT))
      console.log('raw current = ', current)
      current = current * this._current_lsb / 1000

      this._writeWord(REG_CALIBRATION, this._cal_value)
      let power = this._twosComp(this._readHL(REG_POWER))
