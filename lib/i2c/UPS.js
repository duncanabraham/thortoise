...
const SHUNT_RESISTOR_VALUE =0.1
...
class UPS extends i2cBase {
  ...
  async readData() {
    try {
      this._writeWord(REG_CALIBRATION, this._cal_value)
      let shuntVoltage = this._twosComp(this._readWordLH(REG_SHUNTVOLTAGE))
      console.log('raw shuntVoltage = ', shuntVoltage)
      shuntVoltage = shuntVoltage * 0.01 / 1000

      this._writeWord(REG_CALIBRATION, this._cal_value)
      let busVoltage = (this._readWordLH(REG_BUSVOLTAGE))
      console.log('raw busVoltage = ', busVoltage)
      busVoltage = (busVoltage / 8) * 0.004

      this._writeWord(REG_CALIBRATION, this._cal_value)
      let current = this._twosComp(this._readWordLH(REG_CURRENT))
      console.log('raw current = ', current)
      current = current * SHUNT_RESISTOR_VALUE / 1000

      current = shuntVoltage / SHUNT_RESISTOR_VALUE

      this._writeWord(REG_CALIBRATION, this._cal_value)
      let power = this._twosComp(this._readWordLH(REG_POWER))
      console.log('raw power = ', power)
      power = power * this._power_lsb
      ...
      this.data = {
        shuntVoltage,
        busVoltage,
        inputVoltage: shuntVoltage + busVoltage,
        current,
        power,
