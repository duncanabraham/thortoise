...
class UPS extends i2cBase {
  ...
  async readData() {
    try {
      this._writeWord(REG_CALIBRATION, this._cal_value)
      let shuntVoltage = this._readWord2c(REG_SHUNTVOLTAGE) * 0.01
      this._writeWord(REG_CALIBRATION, this._cal_value)
      let busVoltage = (this._readWord(REG_BUSVOLTAGE))
      console.log('raw bus voltage = ', busVoltage)
      busVoltage = (busVoltage / 8) * 0.004      
      this._writeWord(REG_CALIBRATION, this._cal_value)
      let current = this._readWord2c(REG_CURRENT) * this._current_lsb
      this._writeWord(REG_CALIBRATION, this._cal_value)
      let power = this._readWord2c(REG_POWER) * this._power_lsb
