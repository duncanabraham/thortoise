...
class UPS extends i2cBase {
  ...
  async readData() {
    try {
      this._writeWord(REG_CALIBRATION, this._cal_value)
      let shuntVoltage = (this._twosComp(this._readWord(REG_SHUNTVOLTAGE)) * 0.01) / 1000
      let busVoltage = (this._readWord(REG_BUSVOLTAGE) >> 3) * 0.004
      let current = this._twosComp(this._readWord(REG_CURRENT)) * this._current_lsb
      let power = this._twosComp(this._readWord(REG_POWER)) * this._power_lsb
