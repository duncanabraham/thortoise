...
class UPS extends i2cBase {
  ...
  async readData() {
    try {
      this._writeWord(REG_CALIBRATION, this._cal_value)
      const shuntVoltage = this._twosComp(this.this._readWord(REG_SHUNTVOLTAGE))
      const busVoltage = (this._readWord(REG_BUSVOLTAGE) >> 3) * 0.004
      const current = this._twosComp(this._readWord(REG_CURRENT))
      const power = this._twosComp(this._readWord(REG_POWER))
  
      const shuntVoltageV = shuntVoltage / 1000 // convert to V if needed
      const currentA = current / 1000 // convert to A
      const powerW = power / 1000 // seems to be already in W
