...
const calibrationMax = new Triplet(-5000, -5000, -5000)
const calibrationMin = new Triplet(5000, 5000, 5000)
...
class QMC5883L extends I2cBase {
  ...
  _azimuth () {
    ...
    if (avg.t2 < 0) {
      this.azimuth = roundDeg(this.azimuth + 45)
    } else {
      this.azimuth = roundDeg(this.azimuth - 45)
    }
  }
  ...
  _headingFromBob () {
    let x = this._toSignedInt(this._readLH(this.REG_XOUT_LSB))
    let y = this._toSignedInt(this._readLH(this.REG_YOUT_LSB))
    ...
    x -= (calibrationMax.t1 + calibrationMin.t1) / 2
    y -= (calibrationMax.t2 + calibrationMin.t2) / 2

    /* Rescale the measurement to the range observed. */
    const fx = x / (calibrationMax.t1 - calibrationMax.t1)
    const fy = y / (calibrationMax.t2 - calibrationMax.t2)

    let heading = 180.0 * Math.atan2(fy, fx) / Math.PI
  ...
  observed () {
    // x = close to maxX, y close to maxY/2 = North
    // x = maxX * 0.66, y = close to maxY = East
    // x = maxX * 0.33, y = close to minY = West
    // x = close to min, y = close to min = South
    const avg = this.smoothed.avg()
    const marginX = calibrationMax.t1 * 0.33
    const marginY = calibrationMax.t2 * 0.33
