...
class QMC5883L extends I2cBase {
  ...
  getHeading () {
    ...
    const halfX = (calibrationMax.t1 - calibrationMin.t1) / 2
    const halfY = (calibrationMax.t2 - calibrationMin.t2) / 2
    const quartX = (calibrationMax.t1 - calibrationMin.t1) / 8
    const quartY = (calibrationMax.t2 - calibrationMin.t2) / 8
    this._heading = 0
    if (avg.t1 >= halfX + quartX) {
      this._heading += 1 // North of the line
    } else if (avg.t1 < halfX - quartX) {
      this._heading += 4 // South of the line
    }
    if (avg.t2 <= halfY + quartY) {
      this._heading += 8 // East of the line
    } else if (avg.t2 > halfY - quartY) {
      this._heading += 2 // West of the line
    }
