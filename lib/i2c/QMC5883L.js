...
class QMC5883L extends I2cBase {
  ...
  _azimuth () {
    ...
    const maxDeflectionX = Math.abs(this.calibrationMax.t1 - this.calibrationMin.t1) * 4 // 4 x 90 = 360, stay with me ...
    const maxDeflectionY = Math.abs(this.calibrationMax.t2 - this.calibrationMin.t2) * 4
    const xDeg = roundDeg((this.rawValues.t1 / maxDeflectionX) * 360) // degrees as 90 == north
    const yDeg = roundDeg((this.rawValues.t2 / maxDeflectionY) * 360) // degrees as 270 == north

    // const is90 = (xDeg - yDeg) === 90
    this.upsideDown = (xDeg - yDeg) === -90 ? 1 : 0

    // if (is90) {
    this.azimuth = roundDeg(xDeg + (180 * this.upsideDown))
    // }
  }
