...
class QMC5883L extends I2cBase {
  constructor (options) {
    ...
    this.bearing = 0
    this._init()
    this.running = setInterval(this.read.bind(this), 10)
  }

  _init () {
    this._writeByte(this.REG_CONTROL_2, this.SOFT_RST)
    this.setMode(this.MODE_CONT, this.ODR_100HZ, this.RNG_8G, this.OSR_512)
  }
  ...
  get dataReady () {
    ...
    return (status & this.STAT_DRDY) === this.STAT_DRDY
  }

  read () {
    if (this.dataReady) {
      this.rawValues.t1 = this._readWord2c(this.REG_XOUT_LSB, true)
      this.rawValues.t2 = this._readWord2c(this.REG_YOUT_LSB, true)
      this.rawValues.t3 = this._readWord2c(this.REG_ZOUT_LSB, true)
      this.bearing = coords(this.rawValues.t1, this.rawValues.t2).bearingTo(coords(0, 0))
    }
  }
