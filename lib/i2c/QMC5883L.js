...
const calibrationMax = new Triplet(3400, 2095, 3400)
const calibrationMin = new Triplet(0, -1041, 0)
...
class QMC5883L extends I2cBase {
  ...
  _calibrate () {
    ...
    const x = nDec(roundDeg(((calibrationMin.t1 - this.rawValues.t1) / ranges.t1) * 360), 2) // x is the angle of the magnetic field in the x axis ajusted and rounded to 2 decimal places
    const y = nDec(roundDeg(((calibrationMin.t2 - this.rawValues.t2) / ranges.t2) * 360), 2) // y is the angle of the magnetic field in the y axis ajusted and rounded to 2 decimal places
    const z = nDec(roundDeg(((calibrationMin.t3 - this.rawValues.t3) / ranges.t3) * 360), 2) // z is the angle of the magnetic field in the z axis ajusted and rounded to 2 decimal places
    this.bearing = Math.round(coords(y, x).bearingTo(coords(0, 0))) // (0, 0) is the origin of the magnetic field, ie the North Pole
