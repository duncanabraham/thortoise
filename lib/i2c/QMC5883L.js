...
class QMC5883L extends I2cBase {
  ...
  _calibrate () {
    ...
    const centres = new Triplet((calibrationMax.t1 - calibrationMin.t1) / 2, (calibrationMax.t2 - calibrationMin.t2) / 2, (calibrationMax.t3 - calibrationMin.t3) / 2)
    const x = this.rawValues.t1 - centres.t1
    const y = this.rawValues.t2 - centres.t2
    const z = this.rawValues.t3 - centres.t3
    this.heading = 180 * Math.atan2(y, x) / Math.PI
    this.bearing = roundDeg(this.heading)
