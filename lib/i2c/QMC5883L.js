...
class QMC5883L extends I2cBase {
  ...
  _azimuth () {
    return roundDeg(Math.atan2(this.adjustedValues.y, this.adjustedValues.x) * 180 / Math.PI)
  }
  ...
  _calibrate () {
    ...
    const x = (this.rawValues.t1 - offsets.t1) * scale.t1
    const y = (this.rawValues.t2 - offsets.t2) * scale.t2
    const z = (this.rawValues.t3 - offsets.t3) * scale.t3
