...
class Smooth {
  ...
  avg () {
    ...
    return new Triplet(sum.t1 / this.list.length, sum.t2 / this.list.length, sum.t3 / this.list.length)
  }
}
...
class QMC5883L extends I2cBase {
  ...
  async _read () {
    ...
    if (timeout < 50) {
      ...
      this.rawValues.import(roundDeg(Math.floor(x / 10)), roundDeg(Math.floor(y / 10)), roundDeg(Math.floor(z / 10)))
  ...
  _azimuth () {
    ...
    const maxDeflection = Math.abs(calibrationMax.t1 - calibrationMin.t1) * 4 // 4 x 90 = 360, stay with me ...
    const avg = this.smoothed.avg()
    const xDeg = roundDeg((avg.t1 / maxDeflection) * 360) // degrees as 90 == north
    const yDeg = roundDeg((avg.t2 / maxDeflection) * 360) // degrees as 270 == north
