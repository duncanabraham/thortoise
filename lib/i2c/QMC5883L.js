...
class QMC5883L extends I2cBase {
  constructor (options) {
    ...
    this.bearing = 0
    this._init()
    this.running = setInterval(this.read.bind(this), 100)
  }

  _init () {
    this._writeByte(this.REG_RST_PERIOD, this.MODE_CONT)
    this.setMode(this.MODE_CONT, this.ODR_10HZ, this.RNG_2G, this.OSR_256)
  }
  ...
  read () {
    if (this.dataReady()) {
      this.rawValues.t1 = this._readWord2c(this.REG_XOUT_LSB)
      this.rawValues.t2 = this._readWord2c(this.REG_YOUT_LSB)
      this.rawValues.t3 = this._readWord2c(this.REG_ZOUT_LSB)
