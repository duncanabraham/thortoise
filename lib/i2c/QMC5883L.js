...
class QMC5883L extends I2cBase {
  ...
  getHeading () {
    ...
    const quartX = (calibrationMax.t1 - calibrationMin.t1) * 0.66
    const quartY = (calibrationMax.t2 - calibrationMin.t2) * 0.66
    const minX = calibrationMin.t1
    const minY = calibrationMin.t2
    const maxX = calibrationMax.t1
    const maxY = calibrationMax.t2
    this._heading = 0
    if (avg.t1 > maxX - quartX) {
      this._heading += 1 // North of the line
    } else if (avg.t1 < minX + quartX) {
      this._heading += 4 // South of the line
    }
    if (avg.t2 < minY + quartY) {
      this._heading += 8
    } else if (avg.t2 > maxY - quartY) {
      this._heading += 2
    }
