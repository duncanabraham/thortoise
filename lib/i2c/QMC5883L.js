...
class QMC5883L extends I2cBase {
  ...
  getHeading () {
    ...
    this._heading = 0
    if (avg.t1 > (calibrationMax.t1 - calibrationMin.t1) / 4) {
      this._heading += 1 // North of the line
    } else if (avg.t1 < (calibrationMax.t1 - calibrationMin.t1) / 4) {
      this._heading += 4 // South of the line
    }
    if (avg.t2 > (calibrationMax.t2 - calibrationMin.t2) / 4) {
      this._heading += 2 // East of the line
    } else if (avg.t2 < (calibrationMax.t2 - calibrationMin.t2) / 4) {
