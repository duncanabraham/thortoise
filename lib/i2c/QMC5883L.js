...
class QMC5883L extends I2cBase {
  ...
  _calibrate () {
    ...
    const x = nDec((this.rawValues.t1 / ranges.t1) * 360, 2)
    const y = nDec((this.rawValues.t2 / ranges.t2) * 360, 2)
    const z = nDec((this.rawValues.t3 / ranges.t3) * 360, 2)
    this.bearing = Math.round(coords(y, x).bearingTo(coords(0, 0)))
