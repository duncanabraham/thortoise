...
process.env.ENVIRONMENT = 'live' // TODO: Fix dotenv
...
if (process.env.ENVIRONMENT === 'live') {
  ...
  log.info('Running in the DEVELOPMENT Environment: ', process.env)
}

const i2cBus = 0x01 // TODO: Should get this from config / .env
...
class I2cBase extends Feature {
  ...
  _readByte (register) {
    ...
    if (this._openI2C()) {
      try {
        ...
        // console.log(`Reading from register ${register}, got value ${value}`)
      } catch (e) {
        log.info('register: ', register, '    value: ', value)
        log.error(e)
        value = 0
  ...
  _readWordLH (register) {
    ...
    // console.log(`Reading from register ${register}, got value ${result}`)
   ...
   * @returns signed int
   */
  _toSignedInt (n) { // TODO: same as _twosComp() ^
    if (n > 32667) {
      n = n - 65535
    }
    if (n < -32766) {
