...
class PCA9685BASE extends i2cBase {
  ...
  async setPWMFreq (freq) {
    ...
    this._writeByte(this.registers.PRESCALE, 0x79)
    await delay(1)
  ...
  setPWM (channel, on, off) {
    ...
    this._writeWordLH(this.registers.LED0_ON_L + 4 * channel, on)
    this._writeWordLH(this.registers.LED0_OFF_L + 4 * channel, off)
  }
  ...
  setAngle (channel, angle) {
    const count = angle / 180 * 2048

    this.setPWM(channel, 0, count)
  }

  setCount (channel, count) {
    this.setPWM(channel, 0, 200)
  }
}
