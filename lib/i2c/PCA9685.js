...
class PCA9685BASE extends i2cBase {
  ...
  _stop () {
    ...
    this._writeByte(this.registers.ALLLED_ON_H, 0x00)
    this._writeByte(this.registers.ALLLED_ON_L, 0x00)
  }

  setAngle (channel, angle) {
    const pulseUs = NEUTRAL_PULSE + angle * ((MAX_PULSE - MIN_PULSE) / (2 * MAX_ANGLE))
    const periodUs = 1000000 / GLOBAL_FREQUENCY
    const count = (pulseUs / periodUs) / 4096
