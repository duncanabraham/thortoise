...
class ICM20948 extends i2cBase {
  ...
  _writeMag (reg, data) {
    return this.i2cMasterSingleW(this.I2C_ADD_ICM20948_AK09916, reg, data)
  }

  _readMag (reg) {
    return this.i2cMasterSingleR(this.I2C_ADD_ICM20948_AK09916, reg)
  }
  ...
  _i2cMasterReset () {
    this.setBank(this.REG_VAL_REG_BANK_0)
    const register = this.readByte(this.REG_ADD_USER_CTRL)
  ...
  _i2cMasterPassthrough (passthrough) { // Enable or Disable i2c passthrough
    this.setBank(this.REG_VAL_REG_BANK_0)
    let register = this.readByte(this.REG_INT_PIN_CONFIG)
  ...
  getData () {
    ...
    const temperatureH = this._readByte(this.REG_ADD_TEMP_H)
    const temperatureL = this._readByte(this.REG_ADD_TEMP_L)
    this.Temperature = temperatureH + '.' + temperatureL
  }
  ...
  setFullScaleRangeAccel (mode) {
    ...
    let register = this.readByte(this.REG_ADD_ACCEL_CONFIG)
    register &= ~(0b00000110)
    register |= (mode << 1)
  ...
  setFullScaleRangeGyro (mode) {
    this.setBank(this.REG_VAL_REG_BANK_2)
    let register = this.readByte(this.REG_ADD_GYRO_CONFIG_1)
    register &= ~(0b00000110)
    register |= (mode << 1)
  ...
  async _begin () {
    this.setBank(this.REG_VAL_REG_BANK_0)
    const chipId = this.readByte(this.REG_ADD_WIA)
    console.log('Chip ID: ', chipId, ' Valid id is 0xEA')
    await this.swReset()
    this.sleep(false)
    this.lowPower(false)
