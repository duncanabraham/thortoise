...
class ICM20948 extends i2cBase {
  ...
  icm20948GyroAccelRead() {
    ...
    const gyroX = this._readWord(this.REG_ADD_GYRO_XOUT_H)
    const gyroY = this._readWord(this.REG_ADD_GYRO_YOUT_H)
    const gyroZ = this._readWord(this.REG_ADD_GYRO_ZOUT_H)
  ...
  async icm20948ReadSecondary(u8I2CAddr, u8RegAddr, u8Len) {
    ...
    this._writeByte(this.REG_ADD_I2C_SLV0_ADDR, u8I2CAddr)
    this._writeByte(this.REG_ADD_I2C_SLV0_REG, u8RegAddr)
    this._writeByte(this.REG_ADD_I2C_SLV0_CTRL, this.REG_VAL_BIT_SLV0_EN | u8Len)

    this._writeByte(this.REG_ADD_REG_BANK_SEL, this.REG_VAL_REG_BANK_0)

    u8Temp = this._readByte(this.REG_ADD_USER_CTRL)
    u8Temp |= this.REG_VAL_BIT_I2C_MST_EN
    this._writeByte(this.REG_ADD_USER_CTRL, u8Temp)
    await delay(100)
    ...
    for (let i = 0; i < u8Len; i++) {
      this.pu8data[i] = this._readByte(this.REG_ADD_EXT_SENS_DATA_00 + i)
    }

    this._writeByte(this.REG_ADD_REG_BANK_SEL, this.REG_VAL_REG_BANK_3)

    u8Temp = this._readByte(this.REG_ADD_I2C_SLV0_CTRL)
    u8Temp &= ~((this.REG_VAL_BIT_I2C_MST_EN) & (this.REG_VAL_BIT_MASK_LEN))
    this._writeByte(this.REG_ADD_I2C_SLV0_CTRL, u8Temp)

    this._writeByte(this.REG_ADD_REG_BANK_SEL, this.REG_VAL_REG_BANK_0)
  }

  icm20948MagCheck() {
    this.icm20948ReadSecondary(this.I2C_ADD_ICM20948_AK09916 | this.I2C_ADD_ICM20948_AK09916_READ, this.REG_ADD_MAG_WIA1, 2)
    const bRet = (this.pu8data[0] == this.REG_VAL_MAG_WIA1) && (this.pu8data[1] == this.REG_VAL_MAG_WIA2)
    return bRet
  }
  ...
  async icm20948WriteSecondary(u8I2CAddr, u8RegAddr, u8data) {
    let u8Temp = 0
    this._writeByte(this.REG_ADD_REG_BANK_SEL, this.REG_VAL_REG_BANK_3)
    this._writeByte(this.REG_ADD_I2C_SLV1_ADDR, u8I2CAddr)
    this._writeByte(this.REG_ADD_I2C_SLV1_REG, u8RegAddr)
    this._writeByte(this.REG_ADD_I2C_SLV1_DO, u8data)
    this._writeByte(this.REG_ADD_I2C_SLV1_CTRL, this.REG_VAL_BIT_SLV0_EN | 1)
  ...
  async _initialize() {
    ...
    this._writeByte(this.REG_ADD_PWR_MGMT_1, this.REG_VAL_RUN_MODE)
    ...
    await this.icm20948WriteSecondary(this.I2C_ADD_ICM20948_AK09916 | this.I2C_ADD_ICM20948_AK09916_WRITE, this.REG_ADD_MAG_CNTL2, this.REG_VAL_MAG_MODE_20HZ)
  ...
  async icm20948MagRead() {
    ...
    while (counter > 0 || ended) {
      await delay(10)
      this.icm20948ReadSecondary(this.I2C_ADD_ICM20948_AK09916 | this.I2C_ADD_ICM20948_AK09916_READ, this.REG_ADD_MAG_ST2, 1)
    ...
    if (counter != 0) {
      for (let i = 0; i < 8; i++) {
        this.icm20948ReadSecondary(this.I2C_ADD_ICM20948_AK09916 | this.I2C_ADD_ICM20948_AK09916_READ, this.REG_ADD_MAG_DATA, this.MAG_DATA_LEN)
