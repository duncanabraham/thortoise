...
class ICM20948 extends i2cBase {
  ...
  icm20948GyroAccelRead() {
    ...
    accel[0] = this._twosComp(accelX)
    accel[1] = this._twosComp(accelY)
    accel[2] = this._twosComp(accelZ)

    gyro[0] = this._twosComp((gyroX) - this.gyroOffset[0])
    gyro[1] = this._twosComp((gyroY) - this.gyroOffset[1])
    gyro[2] = this._twosComp((gyroZ) - this.gyroOffset[2])
  }
  ...
  async icm20948ReadSecondary(u8I2CAddr, u8RegAddr, u8Len) {
    ...
    for (let i = 0; i < u8Len; i++) {
      pu8data[i] = this._readByte(this.REG_ADD_EXT_SENS_DATA_00 + i)
    }
  ...
  icm20948MagCheck() {
    ...
    const bRet = (pu8data[0] == this.REG_VAL_MAG_WIA1) && (pu8data[1] == this.REG_VAL_MAG_WIA2)
    return bRet
  }
  ...
  async _initialize() {
    ...
    await self.icm20948WriteSecondary(I2C_ADD_ICM20948_AK09916 | I2C_ADD_ICM20948_AK09916_WRITE, REG_ADD_MAG_CNTL2, REG_VAL_MAG_MODE_20HZ)
  }
