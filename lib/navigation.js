...
class Navigation extends Grid {
  constructor (options = { length: 9, width: 9, startX: 5, startY: 5 }) {
    super(options)
    Object.assign(this, options)
    // this.imu = new ICM20948() // we're going to use this module to give us a bearing
    this.centreX = Math.round(this.width / 2)
    this.centreY = Math.round(this.length / 2)
    ...
    this.geoPoint = { lat: 0, lon: 0 }
    // this.compass = new QMC5883L({ i2cAddress: 0x0d })
    this.imu = global.registry.itemByName('ICM20948')
    // this.gps = new GPS()
  }
  ...
  getValues () {
    // this.gpsData = this.gps.getValues()
    // this.geoPoint = this.gpsData.geoPoint
    this.currentHeading = this.imu.lastResult.heading
    // this.compassTemperature = this.compass.temperature
    return {
