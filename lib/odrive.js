...
class ODrive {
  constructor(uartId, baudRate = 9600) {
  ...
  async init() {
    ...
    for (const motor of axis) {
      ...
      await this.write(`w ${motor}.controller.input_mode 1\n`)
      await this.write(`w ${motor}.controller.control_mode 2\n`) // 2 = VELOCITY_CONTROL, 3 = POSITION_CONTROL
  ...
  async clearErrors() {
    ...
    for (const motor of axis) {
      await this.write(`w ${motor}.error 0\n`)
    }
  }
  ...
  async setSpeed(motor, speed) {
    await this.write(`w axis${motor}.controller.input_vel ${speed}\n`)
  }
  ...
  async enableClosedLoop() {
    for (const motor of axis) {
      await this.write(`w ${motor}.requested_state ${axisStateT.AXIS_STATE_CLOSED_LOOP_CONTROL}\n`)
    }
  }
}
