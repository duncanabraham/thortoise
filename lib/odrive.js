...
const axis = ['axis0', 'axis1'] // I know, just go with it
...
class ODrive {
  ...
  async init() {
    console.log('Initialising ODRIVE Motor Controller')
    ...
    for (const motor of axis) {
      await this.write(`${motor}.encoder.config.ignore_illegal_hall_state True\n`)
      await this.write(`${motor}.encoder.config.mode 0\n`) // 0 = ENCODER_MODE_INCREMENTAL, 1 = ENCODER_MODE_HALL
      await this.write(`${motor}.encoder.config.cpr ${odriveSettings.ENCODER_CPR}\n`)
      await this.write(`${motor}.encoder.config.calib_range 0.05\n`)
      await this.write(`${motor}.encoder.config.bandwidth ${odriveSettings.BANDWIDTH}\n`)
      await this.write(`${motor}.encoder.config.zero_count_on_find_idx False\n`)
      await this.write(`${motor}.motor.config.current_lim ${odriveSettings.CURRENT_LIMIT}\n`)
      await this.write(`${motor}.motor.config.current_control_bandwidth ${odriveSettings.BANDWIDTH}\n`)
      await this.write(`${motor}.controller.input_mode 1\n`)
      await this.write(`${motor}.controller.control_mode 2\n`) // 2 = VELOCITY_CONTROL, 3 = POSITION_CONTROL
      await this.write(`${motor}.controller.config.vel_limit ${odriveSettings.VEL_LIMIT}\n`)
      await this.write(`${motor}.controller.config.pos_gain ${odriveSettings.POS_GAIN}\n`)
      await this.write(`${motor}.controller.config.vel_gain ${odriveSettings.VEL_GAIN}\n`)
      await this.write(`${motor}.controller.config.vel_integrator_gain ${odriveSettings.INTEGRATOR}\n`)
    }
  ...
  async disableMotors() {
    for (const motor of axis) {
      await this.write(`${motor}.requested_state ${axisStateT.AXIS_STATE_IDLE}\n`)
    }
  }

  async clearErrors() {
    for (const motor of axis) {
      await this.write(`${motor}.error 0\n`)
    }
  }

  async zeroMotors() {
    for (const motor of axis) {
      await this.write(`${motor}.controller.input_vel 0\n`)
      await this.write(`${motor}.controller.input_pos 0\n`)
    }
  }
  ...
  async enableClosedLoop() {
    for (const motor of axis) {
      await this.write(`${motor}.requested_state ${axisStateT.AXIS_STATE_CLOSED_LOOP_CONTROL}\n`)
    }
  }
}
