...
const anglesFromPosition = (xyz, t1, femurLength, tibiaLength) => {
  ...
  const Q2 = Math.acos((xSqrd + ySqrd - L1Sqrd - L2Sqrd) / (2 * femurLength * tibiaLength))
  const Q1 = Math.atan(y / x) - Math.asin((tibiaLength * Math.sin(Q2)) / Math.sqrt(xSqrd + ySqrd))
  return new Triplet(t1, Q1, Q2)
}
