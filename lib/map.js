...
class Map3D {
  ...
  addPoint (newPoint, options) {
    ...
    newPoint.setContent(options)
  }

  // make the map data available to the map viewer
  publishData () {
    // TODO: I think I need to change lib/redisPubSub.js so that is maintains a pub and a sub client
    // so I can keep the clients separate
    this.mapChanged(true)
    try {
      this.redisClient.pub.publish(this.channel, JSON.stringify(Array.from(this.points.values()).map(p => p.toArray())), (err) => {
        if (err) console.error('catch: ', err)
      })
    } catch (e) {
      console.error(e)
    }
  ...
  // Restore the world from raw data
  loadWorld (pointsData) {
