...
class Servo {
  constructor (options, driver) {
    this.driver = driver
    const defaults = {
      position: 0, // default position can be overridden by the options
      ms: 1000,
      maxPos: 120,
      minPos: 0
    }
  ...
  setPosition (p = 0) { // 0 = Max, 1 = halfway, 2 = 0
    ...
    const position = constrain(range - ((range / 2) * p) + this.minPos, this)
    console.log('setting position: ', position)
    return this.setAbsolutePosition(position)
  }
  ...
  _makeMove () {
    console.log(`moving "${this.name}" from ${this.originalPosition} to ${this.desiredPosition}, currently at ${this.position}`)
    if (this.position >= this.desiredPosition || this.position < this.minPos || this.position > this.maxPos) {
      console.log('You have reached your destination')
      clearInterval(this.timingLoop)
      delete this.timingLoop
      if (this.moveResolve) {
        return this.moveResolve()
      }
    }
