...
class Servo {
  constructor(options, driver) {
    ...
    this.totalSteps = this.steps ? (1 / this.steps) - 1 : 1
  }
  ...
  setSteps(s) {
    this.steps = s
    this.totalSteps = (1 / this.steps) - 1
  }
  ...
  makeMove() {
    ...
    this.stepCount++
    if (this.debug) {
      console.log(this.name, pad(this.originalPosition,3), pad(this.position,3), pad(this.desiredPosition, 3), pad(this.stepCount, 3), pad(this.totalSteps, 3))
    }
  }
