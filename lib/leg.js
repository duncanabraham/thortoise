...
class Leg {
  ...
  _setJoints (t) {
    const originalPosition = new Triplet(this.position.hip, this.position.femur, this.position.knee)
    this.position = t
    const desiredPosition = t
    ...
    return new Promise((resolve) => { // poll to see if the joints reach their destination
      ...
      (function waitForLegs () {
        if (currentPosition.equals(desiredPosition || pollCount > 1000)) return resolve()
        pollCount++
