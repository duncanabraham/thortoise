...
const { move: kineMove } = require('./kinematics')
...
class Leg {
   ...
   */
  home() {
    this.position.t1 = this.servos.hip.startAt
    this.position.t2 = this.servos.femur.startAt
    this.position.t3 = this.servos.knee.startAt
    this._move()
  }

  nextStep(direction, tick) {
    switch (direction) {
      case 'forward':
        this.position.setAngles(kineMove(this.position, tick))
        break
      case 'backward':
        this.position.setAngles(kineMove(this.position, tick - 1))
        break
    }
    this._move()
  }
  ...
  _move() {
    ...
    if (this.name === 'front-left') {
      console.log(pad(this.name, 12, false), pad(t1, 6), pad(t2, 6), pad(t3, 6))
    }
