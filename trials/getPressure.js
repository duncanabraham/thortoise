...
setInterval(() => {
  ...
  if ((status & 0x02) !== 0) {
    ...
    const pressureHPa = pressureRaw / 4096.0
    ...
    const temperatureC = (42.5 + (temperatureRaw / 480.0))

    // Output the pressure and temperature
    log.info(`Pressure (hPa): ${pressureHPa.toFixed(2)}, Temperature (°C): ${temperatureC.toFixed(2)}`)
  } else {
